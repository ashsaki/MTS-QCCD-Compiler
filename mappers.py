...
class QubitMapGreedy:
    def __init__(self, parse_obj, machine_obj):
        self.parse_obj = parse_obj # may be the program
        self.machine_obj = machine_obj # TI QC with information about traps
        ...
        for i in range(len(machine_obj.traps)):
            self.mapping.append([]) # add an empty list for each trap
            self.remaining_capacity.append(trap_capacity)
        self.router = BasicRoute(machine_obj)
        # print(f"remaining cap {self.remaining_capacity}")
    ...
    def _is_mapped(self, qubit):
        """Checks if a qubit is already allocated in any trap."""
    ...
    def _trap(self, qubit):
        """Checks in which trap the qubit is."""
    ...
    # def _map_qubit(self, qubit):
    #     map_order = range(6)
    #     for trap in map_order:
    #         if not self.remaining_capacity[trap] == 0:
    #             return trap
    #         else:
    #             continue
